...
//                                 dark:bg-form-input dark:text-white ${meta.touched && meta.error ?
//                                     "!border-[#FF0000] dark:!border-[#FF0000]" : state.isFocused ?
//                                         "!border-blue-700 dark:!border-blue-700" : "!border-[#A3A3A3]"
//                                 }`,
...
//                         onBlur={() => helpers.setTouched(true)}
//                         inputClass={`!w-full !rounded-lg !border-[1.5px] !py-2.5 !min-h-[41px]
...
//                         }
//                         onBlur={() => helpers.setTouched(true)}
//                         format="yyyy-MM-dd" placeholder={placeholder || "YYYY-MM-DD"}
...
//                         onChange={(date) => {
//                             if (date) {
//                                 const localDateTime = date.toISOString().split("T")[0] + " " +
...
//                         }
//                         onBlur={() => helpers.setTouched(true)}
//                         format={timeFormat24 ? "HH:mm" : "hh:mm aa"}
//                         placeholder={placeholder}
//                         className={`w-full ${meta.touched && meta.error ? `[&_.rs-input-group]:!border-[#FF0000]
//                             dark:[&_.rs-input-group]:!border-[#FF0000]` : ""} focus:[&_.rs-input-group]:!border-blue-700`}
//                     />
...
// static table implementation for  react quill-new
import { ErrorMessage, useField } from "formik";
...
const CustomInput = ({
  ...
  // --- FIX 1: Load from localStorage only when `name` changes
  useEffect(() => {
    const stored = localStorage.getItem(name + "_options");
    if (stored) {
      const parsed = JSON.parse(stored);
      setCustomOptions(prev => (sameOptions(prev, parsed) ? prev : parsed));
    } else {
      // fallback to provided options only once here; updates handled below
      setCustomOptions(prev => (sameOptions(prev, options) ? prev : options));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [name]);

  // --- FIX 2: Apply parent `options` only when content actually changes
  const optionsKey = useMemo(
    () => JSON.stringify((options || []).map(o => [o?.value, o?.label])),
    [options]
  );
  useEffect(() => {
    setCustomOptions(prev => (sameOptions(prev, options) ? prev : (options || [])));
  ...
  const handleAddOption = (inputValue) => {
    ...
    setCustomOptions(prev => {
      if (prev.some(o => o.value === newOption.value)) return prev;
  ...
  // Table helpers (Quill v2 table API)
  ...
  const renderField = () => {
    switch (type) {
      case "description":
        return (
          <div
            className={`rounded-lg border-[1.5px] description ${
              meta.touched && meta.error
                ? "!border-[#FF0000] dark:!border-[#FF0000]"
                : isFocused
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 p-2 border-b border-gray-200 dark:border-form-strokedark">
              <button
                type="button"
            ...
            {/* Plain <style> (no Next.js jsx/global props) */}
            <style>{`
      ...
      case "select":
        return (
          <CreatableSelect
            ...
            classNames={{
              control: (state) =>
                `!min-h-[41px] w-full !rounded-lg border-[1.5px] bg-transparent text-black 
              ...
              singleValue: () => "text-black dark:text-white",
              menu: () =>
                "bg-white dark:bg-form-input text-black dark:text-white rounded-lg shadow-lg mt-1",
              option: (state) =>
                `px-3 py-2 cursor-pointer ${
                  state.isFocused ? "bg-blue-100 dark:bg-blue-700" : ""
                } ${state.isSelected ? "bg-blue-500 text-white" : ""}`,
            }}
          />
        );
      ...
      case "date":
        return (
          <DatePicker
            ...
            className={`w-full ${
              meta.touched && meta.error
                ? `[&_.rs-input-group]:!border-[#FF0000] dark:[&_.rs-input-group]:!border-[#FF0000]`
                : ""
            } focus:[&_.rs-input-group]:!border-blue-700`}
          />
        );
      ...
      case "time":
        return (
          <TimePicker
            ...
            onChange={(val) => helpers.setValue(val ? val.toTimeString().slice(0, 5) : "")}
            onBlur={() => helpers.setTouched(true)}
            format={timeFormat24 ? "HH:mm" : "hh:mm aa"}
            placeholder={placeholder}
            className={`w-full ${
          ...
          <div className="relative flex items-center">
            <input
              ...
              className={`w-full rounded-lg 
                border-[1.5px] py-2.5 bg-transparent text-black outline-none transition focus:border-blue-700
                ps-3 active:border-blue-700 disabled:cursor-default disabled:bg-gray-500 dark:bg-form-input
                dark:focus:border-blue-700 dark:text-white ${
                  meta.touched && meta.error
                    ? `border-[#FF0000] dark:border-[#FF0000]`
                    : "border-[#A3A3A3] dark:border-form-strokedark"
                }`}
  ...
  return (
    <div>
      ...
      <ErrorMessage
        component="div"
        name={field.name}
        className="text-base text-red-600 inter mt-1"
      />
    </div>
  );
};
